...
新增书签后 Worker 仍会异步预热普通外站图标 blob；编辑书签时不再无条件预热图标，避免只改标题、描述、分类或打开方式时重复请求 Favicon.im/Google/自定义图标并写 D1。图标实际变化后的首次显示仍走 `/api/icon/:id` 代理、edge cache、Service Worker cache-first 和短 TTL fallback。
